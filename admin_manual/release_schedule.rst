...
.. note:: You may need to meet new system requirements before the Updater will offer you a new major version. Even if offered, successfully upgrade without encountering problems, there may be other changes required that the Updater cannot check for). Common scenario is needing to upgrade the PHP version, upgrading web server configurations, or typically light adjustments to the Nextcloud `config.php` file. Check the System Requirements section in the Admin Manual for the specific major version you're targeting to make sure your runtime environment is compliant with any new requirements to maximize your chances of a minimal hassle upgrade).
