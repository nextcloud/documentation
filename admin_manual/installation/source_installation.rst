...
The Nextcloud .tar archive contains all of the required PHP modules. This section 
...
* Go to the `Nextcloud Download Page <https://owncloud.org/install>`_.
...
* This unpacks to a single ``owncloud`` directory. Copy the Nextcloud directory 
...
* For Nextcloud to work correctly, we need the module ``mod_rewrite``. Enable it 
  by running::
...
* When using SSL, take special note of the ServerName. You should specify one in 
  ...
  certificate. If you want your Nextcloud to be reachable via the internet, then 
  set both of these to the domain you want to reach your Nextcloud server.
...
.. note:: You can use Nextcloud over plain HTTP, but we strongly encourage you to
...
Nextcloud comes with its own ``owncloud/.htaccess`` file. Because ``php-fpm`` can't 
