...
In Nextcloud 9 the server-side encryption separates encryption of local and 
...
.. note:: Starting with Nextcloud 9 we support Authenticated Encryption for all
   ...
   changes: Never". This will let Nextcloud ignore new files not added via
   Nextcloud, so a malicious external storage administrator could not add new
