...
The :doc:`../file_workflows/access_control` describes how to use the File
Access Control app to deny file operations based on the type of client making
the request. After enabling the app, open the Flow settings in the
administration settings and create a blocking rule with **Request user agent**
set to **Desktop client**.
