...
all your passwords. This example is for documentation only, and you should never use it.
...
domains prevents host header poisoning. Do not remove this, as it performs necessary security checks.
...
not send any language, and the user hasn’t configured their own language preferences.
...
different languages, this value can be set to ``true`` instead of a language code.
...
different languages, this value can be set to ``true`` instead of a locale code.
...
timezone parameter is only used when the timezone of the user cannot be determined.
...
copied to the template directory of new users. Set empty string to not copy any template files.
...
comes with performance benefits as sessions are no longer a locking operation for concurrent requests.
...
passwords or when having a password policy enforcing long passwords (> 300 characters).
...
admin wants to avoid users entering their credentials to the system if the SSO app is unavailable.
...
user is redirected to, when clicking the "Reset password" link after a failed login attempt.
...
server, for example ``nc-admin@example.com``, substituting your own domain, of course.
...
seconds. You may need to increase this if you are running an anti-malware or spam scanner.
...
the automatic detection; for example, ``www.example.com``, or specify the port ``www.example.com:8080``.
...
it may be difficult for Nextcloud to detect this parameter, resulting in invalid URLs.
...
addresses starting with ``10.0.0.`` and ending with 1 to 3: ``^10\.0\.0\.[1-3]$``
...
occ). The value should contain the full base URL: ``https://www.example.com/nextcloud``
...
which can be helpful for them, but can be a privacy issue on small Nextcloud servers.
...
has expired if space is needed. The file will not be deleted if space is not needed.
...
with a unique ID. Only available if ``log_type`` is set to ``syslog`` or ``systemd``.
...
including previews that need to be newly generated, and those that have been generated.
...
If unspecified, defaults to the number of CPU cores, or 4 if that cannot be determined.
...
to require a password. See https://redis.io/topics/security for more information.
...
previews that were generated before Nextcloud 19 in the flat ``appdata_INSTANCEID/previews/FILEID`` folder structure.
...
truncated after collecting the results. I.e., more results can appear than configured here.
...
parameters as described below and run the migration command: ``./occ db:convert-mysql-charset``
...
character set. In some cases, a different collation is desired, such as for accent-sensitive searches.
...
existing files cannot be updated, and no new files can be created in matching folders.

The ``/`` and ``\`` characters, as well as ASCII characters [0-31], are always forbidden.

Example for Windows: ``['?', '<', '>', ':', '*', '|', '"']`` See: https://en.wikipedia.org/wiki/Comparison_of_file_systems#Limits
...
valid ITheme ID, e.g., ``dark``, ``dark-highcontrast``, ``default``, ``light``, ``light-highcontrast``, ``opendyslexic``.
...
identified through scanner/change-detection reconciliation paths (global: all storages).
...
``trusted_proxies``. For example, use ``HTTP_X_FORWARDED_FOR`` for the ``X-Forwarded-For`` header.
...
entries, potentially causing performance degradation and large log files on busy instances.
...
production, as it disables minification and outputs additional debug information.
...
reset to prevent password leaks on public devices if the user forgets to clear it.
...
defined in ``OCP\Accounts\IAccountManager``. Values are merged with defaults from ``OC\Accounts\AccountManager``.

Example: Set phone property to private scope: ``[\OCP\Accounts\IAccountManager::PROPERTY_PHONE => \OCP\Accounts\IAccountManager::SCOPE_PRIVATE]``
