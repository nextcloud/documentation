...
The following applies to all scanner modes, including ICAP:
...
Three settings bound how much of a file is sent to the scanner, and all three
change what the verdict is actually based on:
...
``av_stream_max_length``
  Default ``26214400`` (25 MiB). A file larger than this is not sent in a
  single scan: the scan is finished and evaluated, and a new one is started
  for the following bytes. Each segment is scanned on its own, and a
  detection in any segment marks the file as infected. Detections that depend
  on seeing a whole object, such as a large archive, can be missed when the
  object spans a segment boundary, so raise this limit rather than lowering it
  if your scanner and the PHP ``memory_limit`` allow for it.
...
* you already operate an enterprise scanning appliance or scanning service
  that speaks ICAP, possibly shared with your mail gateway or proxy. The
...
.. list-table::
   ...
   * - ``av_icap_chunk_size``
     - ``1048576``
     - Number of bytes buffered in memory before they are sent to the ICAP
       server.
...
``av_scan_first_bytes`` (see `Scan limits`_), and the
...
Use the mode your ICAP service supports. If the vendor has no preference, use
``respmod``.
...
.. list-table::
   ...
   * - A virus was found
     ...
     - The upload is rejected and the file is deleted, see
       `How scanning works`_.
   * - The file type or file extension is blocked
     - Infected
     - Treated exactly like a malware detection, so an ICAP scanner can also
       enforce a file type policy for Nextcloud uploads.
   * - The file cannot be scanned, for example a password-protected archive,
       an archive nested too deeply, or a file that cannot be decoded
     - Unscannable
...
``av_block_unreachable``
  ...
  server cannot be reached at all or the file couldn't be scanned, and the user is told
...
.. warning:: Unless ``av_icap_tls`` is enabled, the connection to the ICAP
   server is unencrypted: the contents of every scanned file, the file path and
   the uploader's IP address are sent in plaintext. Only leave TLS off when the connection stays on a network you
...
* **The file contents**, subject to the `Scan limits`_.

* **The file path.** For a file in an account's own storage it has the form
  ``/<account>/files/<path>``, so the account's user ID is part of it.

* **The uploader's IP address**, whenever the scan is triggered by a request.
  Background scans send none.
...
Nextcloud from the scanner except the verdict, and the threat name is stored
and logged with the scan result.
