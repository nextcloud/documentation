...
* The option ``forbidden_filename_basenames`` was added to allow bocking files with specific basenames (the filename
  without extension (before the first dot))
...
Add the new preview provider ``OC\Preview\ImaginaryPDF`` to ``enabledPreviewProviders`` to enable preview generation
with Imaginary for PDF files.
...
Starting with Nextcloud 30.0.12 the monitoring app was adjusted to count the active users in the same way as occ
