...
* Setup checks do not check for the ``X-XSS-Protection`` response header anymore. It has been removed from Nextcloud's
  ``.htaccess`` and you may want to adjust your webserver config to not serve it anymore.
...
During the upgrade to Nextcloud 32 the system address book might become disabled if the amount of system users exceeds
...
Starting with Nextcloud 32.0.1, the preview provider for MP3 files, which reads cover images embedded in the files, is
...
If your S3 backend does not support this, you may see an error such as ``Checksum Type mismatch occurred, expected
checksum Type: null, actual checksum Type: crc32`` in your logs when uploading files.
