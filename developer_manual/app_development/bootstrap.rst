...
Note that the context objects of the methods ``register`` and ``boot`` have different interfaces and thus have different
capabilities appropriate for their stage.
...
Any code that should run once for every Nextcloud process goes into the ``boot`` method of an app's ``Application``
class. Use this mechanism carefully as it could have a negative effect on Nextcloud's overall performance.
...
The code above fetches a fictional *foo manager* to register an app class. The boot context object comes with a
``injectFn`` helper that eases dependency injection inside the ``boot`` method by injecting arguments of a callable:
