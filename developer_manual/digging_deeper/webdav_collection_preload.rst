...
Nextcloud emits a preload event so DAV plugins can fetch data for the collection's
children in one go. The goal is to avoid N+1 database queries by
preloading what your property handlers will need before per-node property
handling starts. In practice this means your plugin can fill a cache up
front and then read from it in the usual ``propFind`` handlers for a faster
...
is emitted, against nodes implementing ``Sabre\DAV\ICollection`` with Depth > 0.

The event may be emitted multiple times for a single request path; plugins should
check their cache to avoid duplicate work.
...
.. code-block:: php
    ...
    class MyDavPlugin extends ServerPlugin {
        ...
        private function preloadCollection(PropFind $propFind, ICollection $collection): void {
            // Only preload when your properties have been requested to avoid unnecessary queries
            $requested = [
        ...
        private function handleGetProperties(PropFind $propFind, \Sabre\DAV\INode $node): void {
            // Read and return values from $this->cache here to avoid per-node queries

            // Handle per-node property loading here, to handle Depth = 0
...
- Depth awareness: The event fires for Depth > 0; design your preload logic so
  that sibling PROPFIND calls don’t repeat heavy queries or perform one query
  per node for too many nodes.

Compatibility
-------------

- The ``preloadCollection`` event is available starting with Nextcloud 32.
  Plugins listening for the event will simply not be invoked on earlier
  versions (no extra guards required).
