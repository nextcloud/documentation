...
This document provides a quick overview of the remote wipe API that should be used by clients to implement the remote
wipe functionality.
...
Once a client receives a 401 or 403 status response it will do a fetch to :code:`<server>/index.php/core/wipe/check` and
set the
...
Once the client has wiped all the required data a POST to :code:`<server>/index.php/core/wipe/success` has to be made
with the token.
